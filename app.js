...
async function playTtsBuffer(blob, run, finish){ if(run!==playbackId || !playing)return; const audio=new Audio(URL.createObjectURL(blob)); ttsAudio=audio; audio.preload='auto'; audio.playbackRate=playbackSpeed; audio.preservesPitch=true; audio.webkitPreservesPitch=true; audio.mozPreservesPitch=true; audio.onended=()=>{URL.revokeObjectURL(audio.src);if(ttsAudio===audio)ttsAudio=null;finish();}; audio.onerror=()=>{URL.revokeObjectURL(audio.src);if(ttsAudio===audio)ttsAudio=null;finish();}; await audio.play(); }
