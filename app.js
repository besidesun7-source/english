...
function highlightCandidates(lesson,sentence){ const candidates=[], add=(term,meaning='')=>{if(typeof term==='string'&&term.trim().length>2&&term.trim().split(/\s+/).length<=5)candidates.push({term:term.trim(),meaning});}; (Array.isArray(lesson.keyTerms)?lesson.keyTerms:[]).forEach(item=>add(typeof item==='string'?item:item?.term,typeof item==='object'?item.meaning||'':'')); const title=String(lesson.title||'').replace(/[…!?.]+$/,'').trim(), words=title.match(/[A-Za-z][A-Za-z'’-]*/g)||[]; for(let size=Math.min(5,words.length);size>=2;size--)for(let start=0;start<=words.length-size;start++)add(words.slice(start,start+size).join(' ')); const patterns=[/\b(?:something came up|at the last minute|on purpose|take care of|go to bed|focus on|up to you)\b/i,/\b(?:am|is|are|was|were|be|been|being)\s+(?:up to|able to|going to|supposed to|used to)\b/i,/\b(?:want|wants|wanted|need|needs|needed|prefer|prefers|preferred|decide|decided|plan|planned)\s+to\s+\w+(?:\s+\w+){0,2}\b/i,/\b(?:may|might|will|can|could|should)\s+(?:\w+\s+){0,2}\w+\b/i,/\b\w+(?:ed|ing)\s+(?:on|for|with|at|to)\s+\w+(?:\s+\w+)?\b/i]; patterns.forEach(pattern=>add(sentence.match(pattern)?.[0])); return candidates; }
function annotatedSentence(lesson){ const sentence=String(lesson.sentence || ''), lowered=sentence.toLocaleLowerCase(), seen=new Set(), found=[]; for(const candidate of highlightCandidates(lesson,sentence)){ const index=lowered.indexOf(candidate.term.toLocaleLowerCase()), key=`${index}:${candidate.term.toLocaleLowerCase()}`; if(index>=0&&!seen.has(key)){seen.add(key);found.push({...candidate,index,end:index+candidate.term.length,score:candidate.term.split(/\s+/).length*100+candidate.term.length+(candidate.meaning?1:0)});} } const match=found.sort((a,b)=>b.score-a.score||a.index-b.index)[0]; if(!match)return escapeHtml(sentence); return escapeHtml(sentence.slice(0,match.index))+termMarkup(sentence.slice(match.index,match.end),match.meaning||lesson.meaning)+escapeHtml(sentence.slice(match.end)); }
