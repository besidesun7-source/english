...
function annotatedSentence(lesson){ const sentence=String(lesson.sentence || ''), candidates=highlightCandidates(lesson,sentence), seen=new Set(), matches=[]; for(const candidate of candidates){ const term=candidate.trim(), index=sentence.toLocaleLowerCase().indexOf(term.toLocaleLowerCase()), key=`${index}:${term.toLocaleLowerCase()}`; if(index>=0&&!seen.has(key)){seen.add(key);matches.push({term,index});} if(matches.length>=2)break; } matches.sort((a,b)=>a.index-b.index); if(!matches.length)return escapeHtml(sentence); let cursor=0, result=''; for(const match of matches){ if(match.index<cursor)continue; result+=escapeHtml(sentence.slice(cursor,match.index))+termMarkup(sentence.slice(match.index,match.index+match.term.length),lesson.meaning); cursor=match.index+match.term.length; } return result+escapeHtml(sentence.slice(cursor)); }
